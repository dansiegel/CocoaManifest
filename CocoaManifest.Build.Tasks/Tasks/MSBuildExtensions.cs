...
namespace CocoaManifest.Build.Tasks
{
    ...
    {
        private static bool IsRunningInsideVS => bool.TryParse(Environment.GetEnvironmentVariable("VSIDE"), out bool vside) && vside;
        ...
        {
            void logger(TraceLevel level, string value)
            {
                switch (level)
                {
